...
import com.labour.lar.widget.LazyViewPager;
import com.labour.lar.widget.MainScrollViewPager;

import java.util.List;

/**
 * 可以配合LazyViewPager使用，实现切换到当前页时加载数据并且切换后保存fragment页的状态
 * @author lx
 *
 */
public class FragmentViewPagerAdapter extends PagerAdapter implements LazyViewPager.OnPageChangeListener {
    ...
    private MainScrollViewPager viewPager; // viewPager对象
    private int currentPageIndex = 0; // 当前page索引（切换之前）

    private OnExtraPageChangeListener onExtraPageChangeListener; // ViewPager切换页面时的额外功能添加接口

    public FragmentViewPagerAdapter(FragmentManager fragmentManager, MainScrollViewPager viewPager , List<Fragment> fragments) {
