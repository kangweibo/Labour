 ...
 * 安全质量管理
 */
public class SecurityManagerFrag extends BaseFragment {
    ...
    private void initData(List<Partytype> partytypes) {
        for (Partytype partytype : partytypes) {
            ...
            imgList.add(R.mipmap.quality_icon);
        }
    ...
    private void getPartytypes() {
        ...
        String url = Constants.HTTP_BASE + "/api/safetypes";
    ...
    private void getParties(int typeid) {
        ...
        String url = Constants.HTTP_BASE + "/api/safeties";
        ProgressDialog dialog = ProgressDialog.createDialog(this.getContext());
        dialog.show();
        ...
        OkGo.<String>post(url).upJson(jsonParams).tag("request_tag").execute(new StringCallback() {
            ...
            public void onSuccess(Response<String> response) {
                ...
                if(jr.getSuccess() == 1){
                    ...
                    JSONArray jsonArray = jr.getJSONArrayData();
                    List<Parties> operteams = JSON.parseArray(JSON.toJSONString(jsonArray), Parties.class);

                    partiesList.clear();
                    partiesList.addAll(operteams);
                    showParties();
                } else {
